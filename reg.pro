...
;	ss = (ss - total(ss)/nels)
	ss = (ss - sfit(ss,1))*mask
;	cc = shift(abs(fft(fft(ss,-1)*w(*,*,i,j),1)),wx/2,wy/2)
	cc = shift(abs(fft(fft(ss,-1)*w(*,*,i,j)*smou,1)),wx/2,wy/2)
...
; scene is (nx,xy) or (nx,ny,nf) scene(s) for which displacements are computed
...
; setup some masks and filters if requested (see Makefile)
mm=fltarr(wx,wy)
mm(*,*)=1
 
smou=fltarr(wx,wy)
smou(*,*)=1
;smou = bwth_bp(wx,wy,0,fix((min([wx,wy])/2-1)*.6))


; condition ref
ref=ref/mean(ref)*mean(scene)
win = doref (ref, mm)
...
;
function reg, scene, ref, kernel, apply_scene=apply_scene,$
